...
def update_game(
    ...
    if recalculate_expiration_date:
        game.expiration_date = datetime.datetime.now() + config.EXPIRATION_TIMEDELTA
        database.add_to_database(game)
    ...
    try:
        ...
        reason = get_game_over_reason(board, claim_draw=claim_draw, both_agreed=both_agreed)
...
def update_ongoing_games() -> None:
    games = (database.session.query(database.Game).filter(database.Game.winner.is_(None)).all())
...
def who_offered_action(game: database.Game) -> int:
    if config.debug:
        if game.action_proposed == constants.ACTION_NONE or not (game.white_accepted_action
                                                                 or game.black_accepted_action):
