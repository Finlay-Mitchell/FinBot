...
def create_database_user(discord_user: discord.User) -> database.User:
    if database.session.query(database.User).filter_by(discord_id=discord_user.id).first() is not None:
        raise RuntimeError(f"User #{discord_user.id} already exists")

    user = database.User(discord_id=discord_user.id, username=get_full_username(discord_user))
