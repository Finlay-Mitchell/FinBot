...
def handle_turn_check(user: database.User, game: database.Game) -> None:
    ...
    if (get_turn(board) == constants.WHITE and user == game.black) or (get_turn(board) == constants.BLACK and
                                                                       user == game.white):
        raise RuntimeError(f"User #{user.discord_id} tried to move on the wrong turn in game #{game.id}")
